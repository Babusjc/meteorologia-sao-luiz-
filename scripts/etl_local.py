    ...
    for file in csv_files:
        try:
            ...
            if 'data' in df.columns:
                ...
                logging.info(f"Coluna 'data' convertida para date, tipo: {type(df['data'].iloc[0]) if len(df) > 0 else 'N/A'}")
            ...
            if 'hora' in df.columns:
                ...
                logging.info(f"Coluna 'hora' convertida para time, tipo: {type(df['hora'].iloc[0]) if len(df) > 0 else 'N/A'}")
    ...
    if dfs:
        ...
        try:
            ...
            if df_to_insert_db.empty:
                logging.warning("Nenhum dado válido para inserção após limpeza de nulos. Nenhuma inserção realizada.")
                # Não chamar db.insert_data se o DataFrame estiver vazio
            else:
                ...
                logging.info(f"Tipos de dados: {df_to_insert_db.dtypes}")
